@extends('layouts.app')
@section('title','View Residential Property')
@section('links')
        ...
        <div class="row">
            ...
            <div class="col-lg-9 col-md-12 col-sm-12">
                <div class="cus-dashboard-right didlex nwdes-sec">
                    <h2> Edit Residential Property</h2>
                    <p>If you wish to add a new beneficiary please add that before adding the asset details.</p>
                    <a href="{{route('user.manage.residentials',[@$will_id])}}" class="top_battn_new">Back to list</a>
                </div>

                @include('includes.message')
                <div class="dash-right-inr">

                    <form id="residentials_form" method="POST" action="{{ route('update.residentials')}}">
                        <input type="hidden" class="rm_form_fild" name="will_id" value="{{@$will_id}}" id="will_id">
                        <input type="hidden" class="rm_form_fild" name="id" value="{{@$residentialDetail->id}}">
                        @csrf
                        <div class="col-lg-12 col-md-12">
                            <div class="row login_rm02 for_dashboard rr_border">

                                <div class="col-lg-12 col-md-12 beneficiaries_headingg">
                                    <h1>Residential Property asset</h1>
                                </div>

                                <div class="col-lg-4 col-md-6">
                                    <div class="form-group">
                                        <label>Name of the property</label>
                                        <input type="text" name="pname" value="{{@$residentialDetail->name}}" class="rm_form_fild" placeholder="Enter here">
                                    </div>
                                </div>
                                ...
                                <div class="col-lg-8 col-md-6">
                                    <div class="form-group">
                                        ...
                                        <input type="text" name="address" value="{{@$residentialDetail->address}}" class="rm_form_fild" placeholder="Enter here">
                                    </div>
                                </div>
                                ...
                                <div class="col-lg-4 col-md-6">
                                    <div class="form-group">
                                        ...
                                        <select class="rm_form_fild" name="residential_type" id="residential_type">
                                            <option value="">Select</option>
                                            <option value="Apartment" {{@$residentialDetail->residential_type == 'Apartment' ? 'selected' : ''}}>Apartment</option>
                                            <option value="Bungalow" {{@$residentialDetail->residential_type == 'Bungalow' ? 'selected' : ''}}>Bungalow</option>
                                            <option value="Plot" {{@$residentialDetail->residential_type == 'Plot' ? 'selected' : ''}}>Plot</option>
                                        </select>
                                    </div>
                                </div>
                                ...
                                {{-- <div class="col-lg-4 col-md-6">
                                    <div class="form-group">
                                        <label>Carpet Area <strong>(sq. ft.)</strong> <span class="text-danger">*</span></label>
                                        <input type="text" name="carpet_area" value="{{@$residentialDetail->carpet_area}}" class="rm_form_fild" placeholder="Enter here" min="1">
                                    </div>
                                </div> --}}

                                <div class="col-lg-4 col-md-6 area_hold">
                                    <div class="form-group">
                                        <label>Area <strong>(Sq. ft/ Acres)</strong></label>
                                        <input type="text" name="plot_size" value="{{@$residentialDetail->plot_size}}" id="area_field" class="rm_form_fild" placeholder="Enter here">
                                    </div>
                                </div>
                                ...
                                <div class="col-lg-4 col-md-6">
                                    <div class="form-group">
                                        ...
                                        <select class="rm_form_fild" name="ownership_type" id="ownership_type">
                                            <option value="">Select</option>
                                            <option value="Individual" {{@$residentialDetail->ownership_type == 'Individual' ? 'selected' : ''}}> Individual </option>
                                            <option value="Joint" {{@$residentialDetail->ownership_type == 'Joint' ? 'selected' : ''}}> Joint </option>
                                        </select>
                                    </div>
                                </div>

                                <div class="col-lg-4 col-md-6">
                                    <div class="form-group per_hold" @if(@$residentialDetail->ownership_type=='Joint') style="display:block;" @else style="display:none;" @endif>
                                        <label>Percentage Holding (If Joint) <span class="text-danger">*</span></label>
                                        <input type="text" name="percentage_holding" id="percentage_holding" value="{{@$residentialDetail->percentage_holding}}" class="rm_form_fild" placeholder="Enter here" min="1" max="100">
                                    </div>
                                </div>
                                ...
                                <div class="col-lg-4 col-md-6">
                                    <div class="form-group">
                                        ...
                                        <input type="text" name="litigation" value="{{@$residentialDetail->litigation}}" class="rm_form_fild" placeholder="Enter here">
                                    </div>
                                </div>
                                ...
                                <div class="col-lg-4 col-md-6">
                                    <div class="form-group">
                                        ...
                                        <input type="text" name="jurisdiction" value="{{@$residentialDetail->jurisdiction}}" class="rm_form_fild" placeholder="Enter here">
                                    </div>
                                </div>

                                <div class="col-lg-4 col-md-6 survey_hold">
                                    <div class="form-group">
                                        <label>Survey Number</label>
                                        <input type="text" name="survey_number" value="{{@$residentialDetail->survey_number}}" id="survey_number" class="rm_form_fild" placeholder="Enter here">
                                    </div>
                                </div>
                                ...
                                <div class="col-lg-12 col-md-12">
                                    ...
                                    @if(@$properties >0)
                                        <a href="{{route('user.manage.commercial',[@$will_id])}}" class="submit_rm bntt_collor" id="">Skip</a>
                                    @else
                                        <a href="{{route('user.add.commercial',[@$will_id])}}" class="submit_rm bntt_collor" id="">Skip</a>
                                    @endif
                                ...
                                @include('includes.download_pdf_btn')
                                    ...
                                    <p>If you wish to add another residential property asset, please click "Update" and add another residential property asset.
                                        If you do not have a residential property asset, you can press the "Skip" button. </p>
                                </div>
...
<script>
    ...
    $('#residential_type111').change(function(){

        var residential_type= $('#residential_type').val();

        if(residential_type == 'Bungalow' || residential_type == 'Plot'){

            $('.area_hold').css('display','block');
            $('.area_hold').slideDown();
            $('#area_field').addClass('required');

            $('.survey_hold').css('display','block');
            $('.survey_hold').slideDown();
            //$('#survey_number').addClass('required');

        }else if(residential_type == 'Apartment'){

            $('.area_hold').slideUp();
            $('#area_field').val(' ');
            $('#area_field').removeClass('required');

            $('.survey_hold').slideUp();
            $('#survey_number').val(' ');
            //$('#survey_number').removeClass('required');
        }
